...
struct VertexOutput {
    ...
    @location(2) diffuse_texture_index: u32,
    @location(3) normal_texture_index: u32,
    @location(4) emissive_texture_index: u32,
    @location(5) illumination: u32,
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let diffuse_color = textureSample(textures, sample, in.tex_coords, in.diffuse_texture_index);

    let sunlight = f32(extractBits(in.illumination, 24u, 4u));

    let sunlight_factor = 0.1 + 0.9 * sunlight / 15.0;

    return vec4(diffuse_color.rgb * sunlight_factor, 1.0);
}
...
pub fn create_terrain_pipeline(device: &wgpu::Device, queue: &wgpu::Queue, bindgroup_layouts: Vec<Option<&BindGroupLayout>>, texture_format: wgpu::TextureFormat) -> RenderPipeline {
    let mut world_shader_path = std::env::current_dir().unwrap();
    world_shader_path.push("./res/client/shaders/chunk_shader.wgsl");

    let world_shader_str = read_to_string(world_shader_path).expect("Unable to find Chunk Shader");
    ...
    let pipeline = device.create_render_pipeline(&RenderPipelineDescriptor {
        ...
        vertex: VertexState {
            ...
            buffers: &[SurfaceVertex::desc(), ChunkDataVertex::desc()]
        },
